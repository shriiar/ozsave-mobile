...
/**
 * Compact, consistent dropdown: looks the same on iOS + Android
 * (no native Picker wheel destroying your layout).
 */
                    ...
                    >
                        ...
                        <View style={[styles.selectFooter, { borderTopColor: borderColor }]}>
                            <Pressable
                                ...
                                style={({ pressed }) => [{ opacity: pressed ? 0.9 : 1, alignSelf: "flex-end" }]}
                            >
                                <Text style={{ color: textColor, fontSize: 14, fontWeight: "600" }}>Close</Text>
                            </Pressable>
                        </View>
                    </View>
                </View>
...
export default function AddCostModal({ open, onClose }: Props) {
    ...
    const T = useMemo(() => {
        ...
        const inputBg = isDark ? "rgba(255,255,255,0.05)" : "rgba(2,6,23,0.04)";
        const primary = "#4F46E5";
        const danger = isDark ? "#FCA5A5" : "#B91C1C";

        return { panel, text, muted, border, inputBg, primary, danger };
    }, [isDark]);
    ...
    function DateField({
        ...
        function onChange(_: DateTimePickerEvent, selected?: Date) {
            // Android fires on dismiss too with undefined
            if (!selected) {
                setShow(false);
                return;
            }
            ...
            <View style={styles.backdrop}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.kav}>
                    <View style={styles.centerWrap}>
                        <View style={[styles.panel, { backgroundColor: T.panel, borderColor: T.border }]}>
                            {/* Header */}
                            ...
                            <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
                                {items.map((item, index) => (
                                    <View key={index} style={[styles.block, { borderColor: T.border }]}>
                                        ...
                                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 14 }}>
                                            ...
                                            <View style={{ flexDirection: "row", gap: 14 }}>
                                                ...
                                                {currentUserId ? (
                                                    <Pressable
                                                        disabled={saving}
                                                        onPress={() => selectJustMeShared(index)}
                                                        style={({ pressed }) => [{ opacity: pressed ? 0.85 : 1 }]}
                                                    >
                                ...
                                {categorized ? (
                                    <Pressable disabled={saving} onPress={addItem} style={({ pressed }) => [{ opacity: pressed ? 0.9 : 1 }]}>
                                        <View style={[styles.addItemBtn, { borderColor: T.border, backgroundColor: T.inputBg }]}>
                            ...
                            <View style={[styles.footer, { borderTopColor: T.border }]}>
                                <Pressable
                                    ...
                                    style={({ pressed }) => [
                                        styles.footerBtn,
                                        { borderColor: T.border, backgroundColor: T.inputBg, opacity: pressed ? 0.9 : 1 },
                                    ]}
                                >
...
const styles = StyleSheet.create({
    ...
    kav: { flex: 1 },
    centerWrap: { flex: 1, padding: 14, justifyContent: "center" },
    ...
    h2: { marginTop: 2, fontSize: 12, fontWeight: "400" },

    toggleRow: { paddingHorizontal: 14, paddingTop: 10, paddingBottom: 6 },
    toggleBtn: {
        paddingVertical: 10,
    ...
    blockSub: { marginTop: 2, fontSize: 12, fontWeight: "400" },
