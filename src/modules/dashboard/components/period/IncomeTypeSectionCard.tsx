...
function titleizeIncomeType(raw: string) {
  const s = String(raw ?? "").trim().toLowerCase();
  if (s === "tfn" || s === "abn") return s.toUpperCase();
  if (!s) return "";
  return s.charAt(0).toUpperCase() + s.slice(1);
}

export type IncomeTypePieItem = {
  incomeType: string;
  amount: number;
  percent: number;
};
...
type Props = {
  title: string;
  emptyMessage: string;
  pie: IncomeTypePieItem[];
  incomeTypeInsights?: IncomeTypeInsights;
};

export function IncomeTypeSectionCard(props: Props) {
      ...
      <BlurView intensity={isDark ? 26 : 40} tint={isDark ? "dark" : "light"} style={styles.card}>
        ...
        <View style={[styles.inner, { borderColor: ui.border }]}>
          ...
          <View style={{ paddingHorizontal: 12, paddingTop: 10 }}>
            <Text style={{ color: ui.text, fontWeight: "700", fontSize: 14 }}>{props.title}</Text>
            {props.incomeTypeInsights?.headline ? (
                  ...
                  >
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      <Text style={{ color: ui.text, fontWeight: "700", fontSize: 12 }} numberOfLines={1}>
                        {titleizeIncomeType(selected.incomeType)}
                      </Text>
                    </View>
                      ...
                      >
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                          ...
                          <Text style={{ color: ui.text, fontWeight: "700", fontSize: 13 }} numberOfLines={1}>
                            {titleizeIncomeType(c.incomeType)}
                          </Text>
                        </View>
                  ...
                  <View style={[styles.top3Box, { borderColor: ui.border, backgroundColor: ui.tileBg }]}>
                    ...
                    <ScrollView style={{ maxHeight: 220 }} showsVerticalScrollIndicator={false}>
                      {top3.map((r, idx) => {
                        ...
                        // income semantics: more income is good (green), less is a caution (red)
                        const deltaColor = !up && !down ? ui.sub : up ? "#10b981" : "#ef4444";

                        return (
                          <View key={`${r.incomeType}-${idx}`} style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 8 }}>
                            <View style={{ flex: 1, minWidth: 0, paddingRight: 10 }}>
                              <Text style={{ color: ui.text, fontWeight: "700", fontSize: 13 }} numberOfLines={1}>
                                {titleizeIncomeType(r.incomeType)}
                              </Text>
